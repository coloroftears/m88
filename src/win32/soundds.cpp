...
bool DriverDS::Init(SoundSource<Sample16>* s,
  ...
  timerid = timeSetEvent(buffer_length / num_blocks, timer_resolution,
                         &DriverDS::TimeProc, reinterpret_cast<DWORD_PTR>(this),
                         TIME_PERIODIC);
...
void CALLBACK
DriverDS::TimeProc(UINT uid, UINT, DWORD_PTR user, DWORD_PTR, DWORD_PTR) {
...
void DriverDS::Send() {
  if (playing && !InterlockedExchange(&sending, true)) {
         ...
         writelength);
    {
      ...
      // Lock
      if (DS_OK !=
          lpdsb->Lock(nextwrite, writelength, (void**)&a1, &al1, (void**)&a2,
                      &al2, 0))
        goto ret;
