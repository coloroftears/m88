...
bool ExternalDevice::Init(const char* dllname,
  ...
  if (devinfo.outport && devinfo.outporttable) {
    for (const int* p = devinfo.outporttable; *p != -1; p++)
      bus->ConnectOut(*p, this,
                      static_cast<Device::OutFuncPtr>(&ExternalDevice::Out));
  }
  if (devinfo.inport && devinfo.inporttable) {
    for (const int* p = devinfo.inporttable; *p != -1; p++)
      bus->ConnectIn(*p, this,
                     static_cast<Device::InFuncPtr>(&ExternalDevice::In));
  }
...
//  MemAcquire
//
bool ExternalDevice::S_MemAcquire(void* h,
                                  uint p,
                                  uint n,
                                  void* r,
                                  void* w,
